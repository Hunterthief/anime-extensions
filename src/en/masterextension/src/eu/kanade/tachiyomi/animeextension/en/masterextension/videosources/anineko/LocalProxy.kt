package eu.kanade.tachiyomi.animeextension.en.masterextension.videosources.anineko
...
class LocalProxy(private val client: okhttp3.OkHttpClient) {
    ...
    private fun handleSocket(socket: Socket) {
        ...
        try {
            ...
            if (requestParsed) {
                try { sendError(socket, 500, e.message ?: "Internal Error") } catch (_: Exception) {}
            }
        } finally {
    ...
    private fun sendResponse(socket: Socket, response: Response, targetUrl: String, encodedHeaders: String, clientRange: String?) {
        ...
        val contentType = if (isMpegTs(stripped)) "video/mp2t" else response.header("Content-Type") ?: "application/octet-stream"
    ...
    private fun writeForwardedHeaders(out: java.io.OutputStream, response: Response, isM3u8: Boolean) {
        val headers = response.headers
        for (i in 0 until headers.size) {
            val name = headers.name(i)
            val value = headers.value(i)
            if (name.equals("Connection", ignoreCase = true) || name.equals("Transfer-Encoding", ignoreCase = true) ||
                name.equals("Content-Type", ignoreCase = true) || name.equals("Content-Length", ignoreCase = true) ||
                (!isM3u8 && name.equals("Content-Range", ignoreCase = true)) || (!isM3u8 && name.equals("Accept-Ranges", ignoreCase = true))) continue
    ...
    private fun parseRange(rangeHeader: String?, size: Int): IntRange? {
        ...
        return when {
            start != null && start >= 0 && start < size -> start..min(end ?: (size - 1), size - 1)
            start == null && end != null && end > 0 -> maxOf(0, size - end)..<size
            else -> null
        }
    }
    ...
    private fun stripPngHeader(data: ByteArray): ByteArray {
        ...
        for (offset in 0 until iMin) {
            if (tsData[offset] == 0x47.toByte() && tsData[offset + 188] == 0x47.toByte()) return tsData.copyOfRange(offset, tsData.size)
        }
        return tsData
    }
    ...
    private fun isMpegTs(data: ByteArray): Boolean {
        ...
        return (0..maxOffset).any { offset ->
            data[offset] == 0x47.toByte() && data[offset + 188] == 0x47.toByte() && data[offset + 376] == 0x47.toByte()
        }
    }
